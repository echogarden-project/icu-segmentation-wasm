...
Then download and extract the ICU source code from [the GitHub release page](https://github.com/unicode-org/icu/releases), like `icu4c-77_1-src.zip`, and its corresponding data file, like `icu4c-77_1-data-bin-l.zip`.

Then create a `build` directory inside of its `source` directory

Then in the `build` directory, compile ICU with WebAssembly target with:
```
...
The main source file's (`icu-segmentation.cpp`) reference to that data may need to be updated from `icudt77l_dat` to the version used.
